...
public class DashBoard {
    ...
    @FXML private Button btnSideFlight;
    @FXML private Button btnSideHotel;
    @FXML private Button btnSideUser;
    ...
    public void navigateTo(String fxmlFile) {
        try {
            ...
            // Set reference to this DashBoard in the new controller
    ...
    public void setCurrentUser(User user) {
        ...
        System.out.println("DashBoard a reçu : " + user.getFirstName() + " " + user.getLastName());
        updateNavbar();
    }

    private void updateNavbar() {
        try {
            FXMLLoader loader = new FXMLLoader(getClass().getResource("/dashboard/navbar.fxml"));
            Parent navbar = loader.load();

            Navbar navbarController = loader.getController();
            navbarController.setCurrentUser(currentUser); // 🔥 Passe le user à la navbar

            mainContainer.setTop(navbar); // Place la navbar
        } catch (IOException e) {
            e.printStackTrace();
        }
    }

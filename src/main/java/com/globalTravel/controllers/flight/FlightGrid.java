...
public class FlightGrid implements Navigatable, FrontNavigatable {
    ...
    private void updateButtonVisibility() {
        for (Node node : flightsGrid.getChildren()) {
            if (node instanceof HBox) {
                HBox flightCard = (HBox) node;
                // Traverse the children of the flight card to find the button box
                for (Node child : flightCard.getChildren()) {
                    if (child instanceof VBox) {
                        VBox buttonBox = (VBox) child;
                        // Check if this is the button box (the last VBox in the flight card)
                        if (buttonBox.getChildren().stream().anyMatch(n -> n instanceof Button)) {
                            // Remove the update and delete buttons if the frontOfficeController is set
                            if (frontOfficeController != null) {
                                buttonBox.getChildren().clear(); // Remove all buttons
                            }
                        }
                    }
                }
    ...
    public void navigateToAirline(ActionEvent actionEvent) {
        if(frontOfficeController != null) {
            frontOfficeController.navigateTo("dashboard/flight/airline-grid.fxml");
        } else {
